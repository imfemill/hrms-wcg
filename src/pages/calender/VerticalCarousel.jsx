...
export default function VerticalCarousel({ currentDate, setCurrentDate }) {
  ...
  return (
    ...
    <div className="swiper-button-prev   flex justify-center items-center rounded-md cursor-pointer">
      <ChevronUpIcon className="h-8 w-8 text-wcg_navy/70 hover:text-wcg_blue" aria-hidden="true" />
    </div>
    ...
    >
      ...
      {months.map((item, index) => (
        <SwiperSlide key={index} className="h-28 cursor-pointer" onClick={() => handleSlideClick(index)}>
    ...
    <div className="swiper-button-next  p-3 flex justify-center items-center rounded-md cursor-pointer">
      <ChevronDownIcon className="h-8 w-8 text-wcg_navy/70 hover:text-wcg_blue" aria-hidden="true" />
    </div>
  </div>
