...
const LeaveCard = ({ leaves }) => {
        ...
        <div className="p-4 gap-2 flex-col flex bg-wcg_blue/10 rounded-xl">
          <span className="flex justify-start gap-2">
            ...
            <span className="text-start">
              <span className="text-base text text-wcg_navy w-10.5/12">{cellInfo?.row?.original?.name}</span>
  ...
  const [rotate, setRotate] = useState("transform duration-700 ease rotate-[1deg]");
  ...
  return (
    <div className="shadow-xl shadow-wcg_navy/20 p-5 block h-fit w-11/12 rounded-lg bg-blue-50">
      <div
        className="flex gap-0 justify-between my-4 rounded-t-lg max"
      >
        ...
        <span className={`bg-wcg_navy/90 text-wcg_gray p-1 px-2 space-y-0 rounded-lg cursor-pointer ${rotate}`} onClick={() => toggleAccordion()}>
          {total}
        </span>
      </div>
        ...
        <div className="w-full text-sm flex flex-col max-h-96 gap-4 max-lg:flex-row max-lg:flex-wrap">
          {
            <div className="w-full">
              <ReactTable columns={columns} data={leaves} type={"TODAYS_LEAVES"} />
            </div>
          }
